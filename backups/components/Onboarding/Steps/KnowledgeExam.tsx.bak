...
const KnowledgeExam: React.FC<KnowledgeExamProps> = ({ onComplete, onSkipToMinting }) => {
  ...
  const [earnedBGL, setEarnedBGL] = useState(() => {
    const saved = localStorage.getItem('examEarnedBGL');
    return saved ? parseInt(saved) : 0;
  });
  const [correctAnswersCount, setCorrectAnswersCount] = useState(() => {
    const saved = localStorage.getItem('examCorrectAnswers');
    return saved ? parseInt(saved) : 0;
  });
  ...
  useEffect(() => {
    ...
    localStorage.setItem('examEarnedBGL', earnedBGL.toString());
    localStorage.setItem('examCorrectAnswers', correctAnswersCount.toString());
    localStorage.setItem('examCurrentPage', currentPage.toString());
  }, [userAnswers, timeLeft, earnedBGL, correctAnswersCount, currentPage]);

  // Clear localStorage when exam is completed
  const clearExamState = () => {
    localStorage.removeItem('examAnswers');
    localStorage.removeItem('examTimeLeft');
    localStorage.removeItem('examEarnedBGL');
    localStorage.removeItem('examCorrectAnswers');
    localStorage.removeItem('examCurrentPage');
  };
